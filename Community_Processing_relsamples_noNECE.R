...
read_csv_and_extract <- function(file_path) {
  ...
  # Function to find common samples between consecutive rows
  find_common_samples <- function(samples_list) {
    if (length(samples_list) == 1) {
      return(samples_list[[1]])
    }
    common_samples <- character()
    for (i in 1:(length(samples_list) - 1)) {
      list1 <- strsplit(samples_list[i], ",")[[1]]
      list2 <- strsplit(samples_list[i + 1], ",")[[1]]
      common_values <- intersect(list1, list2)
      if (length(common_values) > 0) {
        common_samples <- c(common_samples, common_values)
      }
    }
  ...
  df %>%
    group_by(community) %>%
    summarise(
      samples = list(find_common_samples(unique(unlist(strsplit(samples, ", "))))),
    ...
    mutate(samples = sapply(samples, function(x) gsub(" ", "", paste(x, collapse = ",")))) %>%
    mutate(Imgs = sapply(Imgs, function(x) gsub(" ", "", paste(x, collapse = ","))))
}
...
for (subdir in subdirs) {
  ...
  setwd('/Users/m254284/Desktop/Neo4j_Projects/Img_events/ModOpt_bycalc/Integration_relsamples_noNECE')
...
is_in <- function(samples, iavarone_ids, data) {
  sapply(iavarone_ids, function(id) {
    if (id %in% unlist(samples)) {
      "in"
    } else if (any(c("CE", "NE") %in% data[data$Iavarone_ID == id, "MRI.contrast.enhancing.annotation"])) {
      "out"
    } else {
...
# Set directories
integration_dir <- "/Users/m254284/Desktop/Neo4j_Projects/Img_events/ModOpt_bycalc/Integration_relsamples_noNECE"
pathways_dir <- file.path(integration_dir, "Pathways/All")
...
filtered_files <- list.files(integration_dir, pattern = ".csv", full.names = TRUE)
...
process_filtered_file <- function(file_path) {
  ...
  colData <- merged_df[, c("Iavarone_ID", "Patient_ID.x", "MRI.contrast.enhancing.annotation", in_out_cols)]
  ...
  for (i in 1:length(in_out_cols)) {
    ...
    formula <- reformulate(c(feature, "Patient_ID.x", "MRI.contrast.enhancing.annotation"))
    colData_sub <- colData[, c("Iavarone_ID", "Patient_ID.x", "MRI.contrast.enhancing.annotation", feature)]
    ...
    if (length(unique_patients_in) < 2 || length(unique_patients_out) < 2) {
      ...
      formula <- reformulate(c(feature, "MRI.contrast.enhancing.annotation"))  # Use only the feature as the formula
    }
    ...
    dds <- DESeqDataSetFromMatrix(countData_sub, colData_sub, design = formula)
    dds <- DESeq(dds)
    ...
    output_file_name <- file.path(pathways_dir, paste0(base_file_name, "_", comparison, "_GSEA.csv"))
...
# PLOTTING ENRICHMENT BY COMMUNITY ----
# Load required libraries
library(dplyr)
library(stringr)
library(ggplot2)
...
# Set directories
pathways_dir <- "/Users/m254284/Desktop/Neo4j_Projects/Img_events/ModOpt_bycalc/Integration_relsamples_noNECE/Pathways/All"
output_dir <- "/Users/m254284/Desktop/Neo4j_Projects/Img_events/ModOpt_bycalc/Integration_relsamples_noNECE/Pathways"
...
create_bubble_plot <- function(files, group_name) {
  ...
  for (cluster in levels(df_up$Cluster)) {
    ...
    #ggsave(output_pdf, plot, width = 10, height = 10)
  }
  return(df_up)
}
...
# SORTING PATHWAYS FOR BSW CLUSTERS ----
# Load required libraries
library(dplyr)
library(tidyr)
library(ggplot2)
...
ggplot_figure <- ggplot(top_agg_data, aes(x = Cluster, y = Signature, fill = Avg_NES)) +
  ...
  theme_classic() +
  labs(title = "Top 5 Pathways per Cluster",
       x = "Cluster",
       y = "Signature",
...
clean_cluster_sample_list <- lapply(cluster_sample_list, function(samples) {
  gsub("samples_by_comm_BSW_", "", samples)
})
...
ggplot(img_frequencies, aes(x = Cluster, y = Imgs, fill = Frequency)) +
  ...
  labs(title = "Heatmap of Imaging Feature Frequencies per Cluster",
       x = "Cluster",
       y = "Imaging Feature",
       fill = "Frequency") +
  theme_classic()
...
heatmap_plot <- ggplot(mean_intensity_per_cluster, aes(x = Cluster, y = Imgs, fill = MeanIntensity)) +
  ...
  labs(title = "Mean Intensity of Imaging Features per Cluster",
       x = "Cluster",
       y = "Imaging Features",
...
ggplot(heatmap_final, aes(x = Imgs, y = Signature, fill = Correlation, alpha = alpha_value)) +
  ...
  theme_classic() +
  labs(x = "Imaging Features", y = "Signature", title = "Correlation Heatmap with P-Value Threshold") +
